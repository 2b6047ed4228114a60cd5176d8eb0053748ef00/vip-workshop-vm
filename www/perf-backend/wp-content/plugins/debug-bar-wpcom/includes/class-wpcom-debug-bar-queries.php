...
class WPCOM_Debug_Bar_Queries extends Debug_Bar_Panel {
	function init() {
		$this->title( __('Queries (WP.com)', 'debug-bar') );
	}
	...
	function render() {
		global $wpdb, $EZSQL_ERROR, $query_execution_time;
		...
		if ( !empty($wpdb->queries) ) {
			...
			foreach ( $wpdb->queries as $q ) {
				...
				$time_of_execution = isset( $query_execution_time[ $query ] ) ? $query_execution_time[ $query ] : 'n/a';
				...
				$out .= "<li>$query<br/><div class='qdebug'>$debug <span>#{$counter} (" . number_format(sprintf('%0.1f', $elapsed * 1000), 1, '.', ',') . "ms @ " . esc_html( $time_of_execution ) . ")</span></div></li>\n";
			}
			$out .= '</ol>';
		} else {
